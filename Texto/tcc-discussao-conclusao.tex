...
Banca ROS 2 + Docker
\begin{itemize}
    \item O TurtleSim foi utilizado?
    \item Slide 5 e 6 - Podem colocar as imagens dos trabalhos relacionados no texto.
    \item O que dos trabalhos relacionados foi utilizado no trabalho?
    \item O que dos trabalhos relacionados foi utilizado no trabalho?
    \item Ficou claro na apresentação o que eram os patrulheiro
    \item Imagens nos slides dos experimentos devem estar no texto
    \item Gostaria de mais detalhes dos testes de navegação. Não faz sentido ele se perder. O ambiente estava mapeado?
    \item A explicação na apresentação está melhoro que do texto
\end{itemize}}