...
No segundo experimento (Base) se usou simulações utilizando o mapa base sem patrulheiros.
...
No quinto experimento (Patrulheiros + Docker) se usou simulações utilizando o mapa base com patrulheiros, com a stack nav2 e rviz dentro de um container Docker.
...
Estes testes foram realizados utilizando a arena base mas com o uso do docker no robô, o robô realizou o mesmo trajeto que o teste anterior de seguir de uma ponta da arena até a outra, os testes foram realizados enquanto o software Nmon verificava o desempenho do sistema.
...
Nos gráficos de CPU (Figura \ref{fig:GACPU}), se pode ver que a mesma acabou por ser mais exigida quando não se utilizava o Docker, por volta de 1\% da média de uso. 
Nos gráficos de memória RAM (Figura \ref{fig:GAMEM}) se pode ver que a diferença do uso é de cerca de 500 MB a mais quando se  usa o Docker. Este aumento ocorreu por conta da necessidade do Docker de subir o container.
Nos gráficos de rede (Figura \ref{fig:GANET}), se pode ver que o uso do Docker é mais evidente, tendo um pico inicial muito alto, mas que diminui logo em seguida. Este aumento ocorreu por conta da necessidade do Docker em se comunicar, via rede, com os outros tópicos utilizados pelo host.  
...
Nos gráficos de CPU (Figura \ref{fig:GPCPU}), se pode ver que foi exigido bem mais da CPU do sistema, enquanto na arena base era exigido até 30\% da CPU, com os patrulheiros foi exigido 50\%, nesta arena a falta do Docker exigiu mais do sistema, enquanto com o Docker houve poucos picos de uso. 
...
Nos testes de rede (Figura \ref{fig:GPNET}), se pode ver que o uso do Docker é novamente mais evidente, tendo um pico de uso muito alto, que logo diminuiu, mas que ainda ficou mais acima dos testes sem Docker. 
